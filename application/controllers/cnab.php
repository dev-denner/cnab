...
class cnab extends MY_Controller
{
    ...
    private $formaLancamento = false;
    private $bancoOp = null;
...
//CARGA dos SELECTS
    ...
    public function controll()
    {
        ...
        try {
            ...
            $this->formaLancamento = $this->POST['forma_pagto'];

            if ($this->formaLancamento == 10) {
                $this->bancoOp = $this->POST[Model_Processo::BANCO];
            }
...
//---------------------------------- FUNCTIONS DAS INTERFACES -------------------------------------------//
    ...
    private function headerLoteCnab240()
    {
        ...
        $this->cnab240->formaLancamento = $this->formaLancamento;
        $this->cnab240->numInscricaoEmpresa = $this->dadosContaBancaria[Model_Conta_Bancaria::CODEMPRESA];
        $this->cnab240->codConvenioNoBanco = $this->dadosContaBancaria[Model_Conta_Bancaria::CONVENIO];
        $this->cnab240->agenciaEmpresa = $this->dadosContaBancaria[Model_Conta_Bancaria::AGENCIA];
        $this->cnab240->digAgenciaEmpresa = $this->dadosContaBancaria[Model_Conta_Bancaria::DIGITOAG];
    ...
    private function registroCnab240()
    {
        ...
        $options = array('BP.' . Model_Base_Processo::PROCESSO => $this->numeroProcesso,);

        if (is_null($this->bancoOp)) {
            $options['BP.' . Model_Base_Processo::BANCO] = $this->getBancoAtual();
        }

        $base = $this->Model_Base_Processo->getByCriterio($options);
        ...
        foreach ($base as $key => $value) {
            ...
            $i++;
            $this->cnab240->numSequencialArquivo = $i;
            $this->cnab240->numInscricaoFavorecido = $cpf;
            $this->cnab240->logradouroFavorecido = $this->retiraAcentos($value[Model_Base_Processo::LOGRADOURO]);
            $this->cnab240->numeroFavorecido = $value[Model_Base_Processo::NUMERO];
            $this->cnab240->complementoFavorecido = $this->retiraAcentos($value[Model_Base_Processo::COMPL]);
            $this->cnab240->bairroFavorecido = $this->retiraAcentos($value[Model_Base_Processo::BAIRRO]);
            $this->cnab240->cidadeFavorecido = $this->retiraAcentos($value[Model_Base_Processo::CIDADE]);
            $this->cnab240->cepFavorecido = substr($value[Model_Base_Processo::CEP], 0, 5);
            $this->cnab240->complementoCepFavorecido = substr($value[Model_Base_Processo::CEP], 5, 3);
            $this->cnab240->ufFavorecido = $value[Model_Base_Processo::UF];
            $this->cnab240->dataVencimento = $this->getPerguntaDataPag();
            $this->cnab240->valorDocumento = str_replace('.', '', $valor);
            $this->cnab240->codDocumentoFavorecido = $cpf;

            $this->inserirLinha($this->cnab240->segmentoB(), $value[Model_Base_Processo::ID]);
        }
...
//echo '<xmp>', print_r($objPHPExcel), exit;
    ...
    private function setBaseProcesso($linha)
    {
        ...
        foreach ($linha as $key => $value) {
            ...
            $dados[Model_Base_Processo::LOGRADOURO] = strtoupper($value[Model_Funcionario::LOGRADOURO]);
            $dados[Model_Base_Processo::NUMERO] = strtoupper($value[Model_Funcionario::NUMERO]);
            $dados[Model_Base_Processo::COMPL] = strtoupper($value[Model_Funcionario::COMPL]);
            $dados[Model_Base_Processo::BAIRRO] = strtoupper($value[Model_Funcionario::BAIRRO]);
            $dados[Model_Base_Processo::CIDADE] = strtoupper($value[Model_Funcionario::CIDADE]);
            $dados[Model_Base_Processo::UF] = strtoupper($value[Model_Funcionario::UF]);
            $dados[Model_Base_Processo::CEP] = $value[Model_Funcionario::CEP];
    ...
    private function retiraAcentos($texto)
    {
        ...
        $texto = strtr(utf8_decode($texto), utf8_decode('ŠŒŽšœžŸ¥µÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýÿºª¹²³£¢¬§'), 'SOZsozYYuAAAAAAACEEEEIIIIDNOOOOOOUUUUYsaaaaaaaceeeeiiiionoooooouuuuyyoa123ECTS');
